...
def main(page: ft.Page):
    try:
        ...
        # RUTA DB ULTRASIMPLE: El directorio raíz en Flet Android es siempre seguro.
        ...
        # ==========================================================
        # 1. LA MAGIA DEL LAZY LOADING (CARGA PEREZOSA)
        # ==========================================================
        # NO instanciamos el fwv.WebView aquí. Dejamos un texto ligero.
        viewer_view = ft.Column([
            ft.Text("Visor 3D inactivo.\nPulsa '▶️ Compilar' en el editor para encender el motor nativo.", 
                    color="grey500", text_align="center")
        ...
        def render_in_wasm():
            ...
            status_text.value = "Iniciando motor WASM..."
            ...
            try:
                if HAS_WEBVIEW and not page.web:
                    # ==========================================================
                    # 2. ENCENDIDO RETARDADO DEL MOTOR 3D
                    # ==========================================================
                    # Si el visor tiene un texto, significa que el WebView aún no existe
                    if len(viewer_view.controls) == 1 and isinstance(viewer_view.controls[0], ft.Text):
                        assets_path = os.path.join("assets", WASM_ENGINE_FILE)
                        try:
                            with open(assets_path, "r", encoding="utf-8") as f:
                                html_data = f.read()
                        except Exception as e:
                            html_data = f"<html><body style='background:#111;color:#fff;'>Error assets: {e}</body></html>"

                        b64 = base64.b64encode(html_data.encode('utf-8')).decode('utf-8')
                        viewer_view.controls.clear()
                        
                        # ¡AQUÍ NACE EL WEBVIEW NATIVO, SOLO CUANDO SE NECESITA!
                        new_wv = fwv.WebView(url=f"data:text/html;base64,{b64}", expand=True)
                        viewer_view.controls.append(new_wv)
                        page.update()

                    # Una vez encendido, le inyectamos tu código
                    for ctrl in viewer_view.controls:
                        if isinstance(ctrl, fwv.WebView):
                            ctrl.run_javascript(f"processOpenScad('{code}');")
                else:
                    viewer_view.controls[0].value = "Visor 3D: Activo solo en APK."
                    viewer_view.controls[0].color = "yellow"
                    page.update()
