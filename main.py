...
# =========================================================
# APLICACIÓN PRINCIPAL v16.5 (MAKER SUITE & 33 TOOLS)
# =========================================================
def main(page: ft.Page):
    try:
        ...
        status = ft.Text("NEXUS v16.5 PRO | Multi-Core Activo. Ensamblador Listo.", color="#00E5FF", weight="bold")
        ...
        panel_tolerancia = ft.Container(
            content=ft.Column([
                ft.Row([ft.Text("⚙️ CORE: Ajustes", color="#FFAB00", weight="bold", size=11), sw_ensamble]),
                panel_ensamble_ops,
                r_g_tol
        ...
        # =========================================================
        # MAKER SUITE FIX (Bypass de validación on_change Termux)
        # =========================================================
        tf_texto = ft.TextField(label="Escribe Texto", value="NEXUS", max_length=10, bgcolor="#161B22")
        dd_txt_estilo = ft.Dropdown(label="Estilo/Fuente", options=[ft.dropdown.Option("Voxel"), ft.dropdown.Option("Braille")], value="Voxel", expand=True, bgcolor="#161B22")
        dd_txt_base = ft.Dropdown(label="Formato de Base", options=[ft.dropdown.Option("Texto Libre"), ft.dropdown.Option("Llavero (Anilla)")], value="Texto Libre", expand=True, bgcolor="#161B22")
        
        # INYECCIÓN DINÁMICA DE EVENTOS (Previene crashes en Flet ARM)
        tf_texto.on_change = lambda e: generate_param_code()
        dd_txt_estilo.on_change = lambda e: generate_param_code()
        dd_txt_base.on_change = lambda e: generate_param_code()
        
        sl_txt_h, r_txt_h = create_slider("Grosor Z", 1, 20, 5, False, generate_param_code)
        ...
        # =========================================================
        # MOTOR PARAMÉTRICO V16.5 (33 HERRAMIENTAS)
        # =========================================================
...
"""
                if base == "Llavero (Anilla)":
                    code += f"""
  var r_llavero = 6;
  var baseC = CSG.cube({{center:[(totalL/2)-3, 3, h/4], radius:[(totalL/2)+2, 8, h/4]}});
  var anclaje = CSG.cylinder({{start:[totalL, 3, 0], end:[totalL, 3, h/2], radius:r_llavero, slices:32}});
  var agujero = CSG.cylinder({{start:[totalL, 3, -1], end:[totalL, 3, h/2+1], radius:3, slices:16}});
  baseC = baseC.union(anclaje).subtract(agujero);
  if(pText !== null) return baseC.union(pText);
  return baseC;
}}"""
                else:
                    code += f"""
  ...
  return CSG.cube({{center:[0,0,0], radius:[1,1,1]}});
                ...
                code += f"  return slice_2d.scale([1,1,10]);\n}}"
                if not modo_ensamble: txt_code.value = code
        ...
        # =========================================================
        # GESTIÓN DE VISIBILIDAD DE PANELES (UI)
        # =========================================================
        ...
        view_constructor = ft.Column([
            ...
            ft.ElevatedButton("▶ DELEGAR A NÚCLEOS JS (RENDER)", on_click=lambda _: run_render(), color="black", bgcolor="#00E676", height=60, width=float('inf'))
        ...
        view_visor = ft.Column([
            ...
            ft.Text("Motor WebGL / CPU Multi-Core", text_align="center", color="#00E5FF", weight="bold"),
            ft.Row([btn_visor], alignment=ft.MainAxisAlignment.CENTER),
            ft.Container(height=20),
            ft.Text("📦 El progreso del renderizado aparecerá ahora en la pestaña web.", color="#FFAB00", text_align="center", size=12)
        ], expand=True)
