...
def main(page: ft.Page):
    try:
        ...
        # ==========================================
        # CONTROL VARIABLES I18N EN TIEMPO REAL
        # ==========================================
        ...
        lbl_btn_union = ft.Text(lang.t("btn_union"), color="white")
        lbl_btn_subtract = ft.Text(lang.t("btn_subtract"), color="white")

        # Pestaña View 3D / PBR
        lbl_visor_ext = ft.Text(lang.t("lbl_visor_ext"), color="#B388FF", weight="bold", size=11)
        lbl_nexus_pc = ft.Text(lang.t("lbl_nexus_pc"), color="#00E5FF", weight="bold", size=11)
        lbl_btn_open_native = ft.Text(lang.t("btn_open_native"), color="black")
        
        lbl_pbr_studio = ft.Text(lang.t("lbl_pbr_studio"), size=22, color="#FF007F", weight="bold", text_align="center")
        lbl_pbr_desc = ft.Text(lang.t("lbl_pbr_desc"), color="#E6EDF3", text_align="center")
        lbl_btn_open_pbr = ft.Text(lang.t("btn_open_pbr"), color="white")

        # Pestaña Lab / Archivos
        lbl_caliper = ft.Text(lang.t("lbl_caliper"), color="#E6EDF3", weight="bold")
        lbl_internal_db = ft.Text(lang.t("lbl_internal_db"), color="#00E676", weight="bold")
        lbl_btn_web_injector = ft.Text(lang.t("btn_web_injector"), color="white")
        ...
        # Diálogos
        dialog_title = ft.Text(lang.t("dialog_rename"), color="#00E5FF")
        btn_cancel_txt = ft.Text(lang.t("btn_cancel"))
        btn_save_diag_txt = ft.Text(lang.t("btn_save_diag"), color="black")
  ...
  var UTILS = { 
    ...
    mat: function(o) { if(!o) return CSG.cube({radius:[0.01,0.01,0.01]}); try { if(Array.isArray(o)) return o.map(function(x){return UTILS.mat(x);}); if(typeof o.setColor === 'function') return o.setColor(MAT_C); } catch(e) {} return o; } 
        ...
        panel_ensamble_ops = ft.Row([
            ft.ElevatedButton(content=lbl_btn_union, on_click=lambda _: add_to_stack("union"), bgcolor="#1B5E20", expand=True),
            ft.ElevatedButton(content=lbl_btn_subtract, on_click=lambda _: add_to_stack("subtract"), bgcolor="#B71C1C", expand=True)
        ], visible=False)
        ...
        lbl_ensamble_warn = ft.Text("⚠️", color="#FFAB00", weight="bold", visible=False)
        ...
        def add_assembly_part(e):
            files = [f for f in os.listdir(EXPORT_DIR) if f.lower().endswith('.stl') and f != "imported.stl"]
            if not files: status.value = "❌"; status.color = "#FF5252"; page.update(); return
            for i in range(MAX_ASSEMBLY_PARTS):
            ...
            status.value = "❌ Límite (10)"; status.color = "#FFAB00"; page.update()
        ...
        view_ensamble = ft.Column([lbl_assembly, ft.Row([ft.ElevatedButton(content=lbl_btn_add_part, on_click=add_assembly_part, bgcolor="#1B5E20"), ft.ElevatedButton(content=lbl_btn_view_pbr, on_click=lambda _: navigate_to(5), bgcolor="#C51162")]), ft.Divider(), col_assembly], expand=True)
        ...
        def confirm_rename(e):
            ...
            if new_name and new_name != rename_target:
                ...
                try: os.rename(os.path.join(EXPORT_DIR, rename_target), os.path.join(EXPORT_DIR, new_name)); status.value = f"✓"; status.color = "#00E676"
                except Exception as ex: status.value = f"❌ Error"; status.color = "red"
            dialog_rename.open = False; refresh_nexus_db(); page.update()

        dialog_rename = ft.AlertDialog(title=dialog_title, content=tf_rename, actions=[ft.TextButton(content=btn_cancel_txt, on_click=lambda e: [setattr(dialog_rename, 'open', False), page.update()]), ft.ElevatedButton(content=btn_save_diag_txt, on_click=confirm_rename, bgcolor="#00E676")])
        page.overlay.append(dialog_rename)

        list_nexus_db = ft.ListView(height=200, spacing=5)

        def direct_download_file(e, filename):
            try: os.makedirs(DOWNLOAD_DIR, exist_ok=True); shutil.copy(os.path.join(EXPORT_DIR, filename), os.path.join(DOWNLOAD_DIR, filename)); status.value = f"✓"; status.color = "#00E676"
            except Exception as ex: status.value = f"❌ Error"; status.color = "#FF5252"
            page.update()
        ...
        def export_obj_file(e, filename):
            ...
            status.value = f"✓" if success else f"❌"; status.color = "#00E5FF" if success else "#FF5252"; page.update()
        ...
        def refresh_nexus_db():
            ...
            try:
                ...
                if not files: list_nexus_db.controls.append(ft.Text("...", color="#8B949E", italic=True))
                for f in files:
                    ...
                    actions = [custom_icon_btn("✏️", lambda e, fn=f: open_rename_dialog(fn), "Rename"), custom_icon_btn("⬇️", lambda e, fn=f: direct_download_file(e, fn), "Download"), custom_icon_btn("🗑️", lambda e, fp=p: [os.remove(fp), refresh_nexus_db()], "Delete")]
                    if ext == "stl": actions.insert(0, custom_icon_btn("📦", lambda e, fn=f: export_obj_file(e, fn), "OBJ")); actions.insert(0, custom_icon_btn("▶️", lambda e, fp=p: load_file(fp), "Load"))
                    elif ext == "jscad": actions.insert(0, custom_icon_btn("▶️", lambda e, fp=p: load_file(fp), "Load"))
        ...
        def load_file(filepath):
            ...
            if ext == "stl":
                ...
                select_tool("stl"); navigate_to(2); update_code_wrapper(); status.value = f"✓ STL"
            elif ext == "jscad": txt_code.value = open(filepath).read(); navigate_to(1); status.value = "✓ JS"
            page.update()
        ...
        def copy_to_db(e, filepath, filename):
            try: shutil.copy(filepath, os.path.join(EXPORT_DIR, filename)); status.value = f"✓ {filename}"; status.color = "#00E676"; refresh_nexus_db()
            except Exception as ex: status.value = f"❌ Error"; status.color = "red"
            page.update()
        ...
        def refresh_explorer(path):
            ...
            try:
                ...
                if path != "/" and path != "/storage" and path != "/storage/emulated": list_android.controls.append(ft.Container(content=ft.Row([ft.Text("⬆️", size=20), ft.Text("..", color="white", expand=True)]), bgcolor="#30363D", padding=5, border_radius=5, on_click=lambda e: nav_to(os.path.dirname(path)), ink=True))
                for d in dirs:
                    if not d.startswith('.'): list_android.controls.append(ft.Container(content=ft.Row([ft.Text("📁", size=20), ft.Text(d, color="#E6EDF3", expand=True, no_wrap=True, overflow=ft.TextOverflow.ELLIPSIS), custom_icon_btn("➡️", lambda e, p=os.path.join(path, d): nav_to(p), "Go")]), bgcolor="#161B22", padding=5, border_radius=5, on_click=lambda e, p=os.path.join(path, d): nav_to(p), ink=True))
                for f in files:
                    ...
                    actions = [custom_icon_btn("▶️", lambda e, fp=p: file_action(fp), "Load"), custom_icon_btn("📥", lambda e, fp=p, fn=f: copy_to_db(e, fp, fn), "Import")]
        ...
        def save_project_to_nexus():
            ...
            status.value = f"✓ DB: {fname}"; page.update()

        row_quick_paths = ft.Row([ft.ElevatedButton("🏠 Android", on_click=lambda _: nav_to("/storage/emulated/0"), bgcolor="#21262D", color="white"), ft.ElevatedButton(content=lbl_btn_dl, on_click=lambda _: nav_to("/storage/emulated/0/Download"), bgcolor="#21262D"), ft.ElevatedButton("📁 Nexus DB", on_click=lambda _: nav_to(EXPORT_DIR), bgcolor="#1B5E20", color="white")], scroll="auto")
        ...
        def toggle_lang(e):
            ...
            # Navegación
            lbl_nav_studio.value = lang.t("nav_studio"); lbl_nav_view.value = lang.t("nav_view"); lbl_nav_lab.value = lang.t("nav_lab")
            lbl_sub_ia.value = lang.t("btn_ia"); lbl_sub_sliders.value = lang.t("btn_sliders"); lbl_sub_code.value = lang.t("btn_code")
            lbl_sub_3d.value = lang.t("btn_3d"); lbl_sub_pbr.value = lang.t("btn_pbr")
            lbl_sub_db.value = lang.t("btn_db"); lbl_sub_assemble.value = lang.t("btn_assemble")
            ...
            r_g_w.lbl.value = lang.t("sl_w"); r_g_l.lbl.value = lang.t("sl_l"); r_g_h.lbl.value = lang.t("sl_h"); r_g_t.lbl.value = lang.t("sl_t"); r_g_tol.lbl.value = lang.t("sl_tol"); r_kine.lbl.value = lang.t("sl_anim")
            
            lbl_btn_render.value = lang.t("btn_render"); lbl_btn_save.value = lang.t("btn_save"); lbl_btn_reset.value = lang.t("btn_reset")
            lbl_btn_union.value = lang.t("btn_union"); lbl_btn_subtract.value = lang.t("btn_subtract")
            txt_code.label = lang.t("lbl_code")

            # Visualización
            lbl_visor_ext.value = lang.t("lbl_visor_ext"); lbl_nexus_pc.value = lang.t("lbl_nexus_pc"); lbl_btn_open_native.value = lang.t("btn_open_native")
            lbl_pbr_studio.value = lang.t("lbl_pbr_studio"); lbl_pbr_desc.value = lang.t("lbl_pbr_desc"); lbl_btn_open_pbr.value = lang.t("btn_open_pbr")

            # LAB
            lbl_caliper.value = lang.t("lbl_caliper"); lbl_internal_db.value = lang.t("lbl_internal_db"); lbl_btn_web_injector.value = lang.t("btn_web_injector")
            lbl_native_exp.value = lang.t("lbl_native_exp"); lbl_btn_go.value = lang.t("btn_go"); lbl_btn_save_code.value = lang.t("btn_save_code")
            lbl_btn_dl.value = lang.t("btn_dl"); lbl_assembly.value = lang.t("lbl_assembly"); lbl_btn_add_part.value = lang.t("btn_add_part"); lbl_btn_view_pbr.value = lang.t("btn_view_pbr")
            
            # Diálogos
            dialog_title.value = lang.t("dialog_rename"); tf_rename.label = lang.t("tf_new_name")
            btn_cancel_txt.value = lang.t("btn_cancel"); btn_save_diag_txt.value = lang.t("btn_save_diag")

            # Magia oscura: Destruimos y reconstruimos las herramientas para forzar el cambio de idioma
            nonlocal tools_lib, view_constructor
            tools_lib = nexus_ui_tools.NexusTools(create_slider, update_code_wrapper, set_tab_wrapper, select_tool)
            
            view_constructor.controls = [
                panel_globales, 
                lbl_cat_opts, tools_lib.cat_especial,
                lbl_cat_sketch, tools_lib.cat_bocetos,
                lbl_cat_forge, tools_lib.cat_stl_forge,
            ...
            select_tool(herramienta_actual)
            page.update()
