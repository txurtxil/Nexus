...
def main(page: ft.Page):
    try:
        ...
        # Visor inactivo por defecto (Lazy Loading)
        ...
        def render_in_wasm():
            ...
            try:
                if HAS_WEBVIEW and not page.web:
                    ...
                    # INYECCIÓN DIRECTA: Medio segundo de retraso para que el Android cargue el WebView
                    html_data = html_data.replace("initCAD();", f"initCAD(); setTimeout(() => processOpenScad('{code}'), 800);")
                    b64 = base64.b64encode(html_data.encode('utf-8')).decode('utf-8')
                    
                    viewer_view.controls.clear()
                    # FIX CRÍTICO: Sin javascript_enabled
...
if __name__ == "__main__":
    # FIX CRÍTICO: Eliminado el os.makedirs("assets") y auto-detección correcta
