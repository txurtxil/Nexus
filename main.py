...
# Silenciador técnico de advertencias obsoletas de Flet
warnings.simplefilter("ignore", DeprecationWarning)

# Intentamos importar el WebView nativo. Si falla (en PC/Web), cargaremos placeholder
try:
...
def main(page: ft.Page):
    # ---------------------------------------------------------
    # 1. BLINDAJE: try/except total contra pantallazo gris
    # ---------------------------------------------------------
    try:
        # Configuración visual blindada con Emojis (image_2.png approved)
        page.title = "NEXUS 3D Studio"
        page.theme_mode = "dark"
        page.bgcolor = "#0a0a0a" # Uso hex para blindaje
        page.padding = 10

        # RUTA DB ULTRA-BLINDADA PARA ANDROID Sandbox
        home_dir = os.environ.get("HOME")
        if not home_dir or home_dir == "/":
            # Si os.getcwd() o HOME fallan, usamos TMPDIR
        ...
        txt_code = ft.TextField(
            ...
            bgcolor="#000000", color="#00ff00", # Estilo CAD clásico
        ...
        if os.path.exists(assets_path):
            ...
            # Fallback seguro si los assets no se copiaron
            wasm_html_content = "<html><body style='background:#111;color:#fff;'>Visor 3D no encontrado en assets.</body></html>"

        # Cargar HTML como Data URI para asegurar persistencia offline en el APK
        b64_html = base64.b64encode(wasm_html_content.encode('utf-8')).decode('utf-8')
        
        # Componente WebView (Puente a la Opción B)
        # REGLA DE BLINDAJE: Solo se instancia si estamos en el APK o nativo.
        if HAS_WEBVIEW and not page.web:
            wasm_webview = fwv.WebView(
                url=f"data:text/html;base64,{b64_html}",
                expand=True,
                # FIX CRÍTICO: No poner javascript_enabled=True, viene por defecto y crashea.
            )
        else:
            # Texto simulado para cuando pruebas localmente en localhost:8555
        ...
        def load_project_to_editor(name):
            ...
            if row:
                ...
                switch_tab(0) # Volver a pestaña editor
            page.update()
        ...
        # REGLA TÉCNICA SENIOR: Comunicar Python -> WebView JS
        def render_in_wasm():
            # Limpiar comillas simples y saltos de línea para inyectar como JS string
            code = txt_code.value.replace("\\", "\\\\").replace("'", "\\'").replace("\n", "\\n")
            
            # Avisamos visualmente antes de enviar
            status_text.value = "Inyectando código en motor WASM offline..."
            status_text.color = "orange400"
            page.update()
            
            # Cambiamos a la pestaña del Visor 3D instantáneamente
            switch_tab(1)
        ...
        # El WebView nativo o el placeholder ocupando el resto
        ...
        def switch_tab(index):
            ...
            # Cargar historial solo al abrir la pestaña
            if index == 2:
                load_history()
    ...
    # ---------------------------------------------------------
    # 3. BLINDAJE: Diagnóstico rojo ante fallos de Sandbox
    # ---------------------------------------------------------
    except Exception:
        page.clean()
        # Pantalla roja de diagnóstico blindada.
        page.bgcolor = "#990000" 
        page.add(
...
if __name__ == "__main__":
    # EL FIX CRÍTICO: Eliminado el 'os.makedirs("assets")' que crasheaba el APK
