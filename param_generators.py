# param_generators.py
# Módulo de Geometría Procedural NEXUS CAD v22 (GPU OVERDRIVE EDITION)
...
def get_code(h, p):
    ...
    if h == "sketcher":
        ...
        if "Voxel" in estilo:
            ...
            code += f"  var braille = {{ 'A':[1], 'B':[1,2], 'C':[1,4], 'D':[1,4,5], 'E':[1,5], 'F':[1,2,4], 'G':[1,2,4,5], 'H':[1,2,5], 'I':[2,4], 'J':[2,4,5], 'K':[1,3], 'L':[1,2,3], 'M':[1,3,4], 'N':[1,3,4,5], 'O':[1,3,5], 'P':[1,2,3,4], 'Q':[1,2,3,4,5], 'R':[1,2,3,5], 'S':[2,3,4], 'T':[2,3,4,5], 'U':[1,3,6], 'V':[1,2,3,6], 'W':[2,4,5,6], 'X':[1,3,4,6], 'Y':[1,3,4,5,6], 'Z':[1,3,5,6], ' ':[0] }};\n  var pText = null; var stepX = 4; var stepY = 4; var charWidth = 10;\n  for(var i=0; i<texto.length; i++) {{ var dots = braille[texto[i]] || [1]; var offX = i * charWidth; for(var d=0; d<dots.length; d++) {{ var p = dots[d]; if (p === 0) continue; var cx = (p>3) ? stepX : 0; var cy = ((p-1)%3 === 0) ? stepY*2 : (((p-1)%3 === 1) ? stepY : 0); var domo = CSG.sphere({{center:[offX+cx, cy, {z_start}], radius:{rad_braille}, resolution:32}}); if(!pText) pText = domo; else pText = pText.union(domo); }} }}\n  var totalL = Math.max(texto.length * charWidth, 10);\n"
        code += "  if (!pText) pText = CSG.cube({center:[0,0,0], radius:[0.01, 0.01, 0.01]});\n  var baseObj = null;\n"
        if base == "Llavero (Anilla)": code += "  var bc = CSG.cube({center:[(totalL/2)-3, 3, h/4], radius:[(totalL/2)+2, 8, h/4]}); var anclaje = CSG.cylinder({start:[totalL, 3, 0], end:[totalL, 3, h/2], radius:6, slices:64}).subtract(CSG.cylinder({start:[totalL, 3, -1], end:[totalL, 3, h/2+1], radius:3, slices:32})); baseObj = bc.union(anclaje);\n"
        elif base == "Placa Atornillable": code += "  var bc = CSG.cube({center:[totalL/2-3, 3, h/4], radius:[totalL/2+10, 10, h/4]}); var h1 = CSG.cylinder({start:[-8, 3, -1], end:[-8, 3, h], radius:2.5, slices:32}); var h2 = CSG.cylinder({start:[totalL+2, 3, -1], end:[totalL+2, 3, h], radius:2.5, slices:32}); baseObj = bc.subtract(h1).subtract(h2);\n"
        ...
        elif base == "Colgante Militar": code += "  var b_cen = CSG.cube({center:[totalL/2-3, 4, h/4], radius:[totalL/2-1, 10, h/4]}); var b_izq = CSG.cylinder({start:[-4, 4, 0], end:[-4, 4, h/2], radius:10, slices:64}); var b_der = CSG.cylinder({start:[totalL-2, 4, 0], end:[totalL-2, 4, h/2], radius:10, slices:64}); var agujero = CSG.cylinder({start:[-8, 4, -1], end:[-8, 4, h], radius:2.5, slices:32}); baseObj = b_cen.union(b_izq).union(b_der).subtract(agujero);\n"
        elif base == "Placa Ovalada": code += "  var c1 = CSG.cylinder({start:[-2, 4, 0], end:[-2, 4, h/2], radius:12, slices:128}); var c2 = CSG.cylinder({start:[totalL-4, 4, 0], end:[totalL-4, 4, h/2], radius:12, slices:128}); var p_med = CSG.cube({center:[totalL/2-3, 4, h/4], radius:[totalL/2-1, 12, h/4]}); baseObj = p_med.union(c1).union(c2);\n"
        code += "  if(baseObj) {\n"
    ...
    # -----------------------------------------
        ...
        if h == "stl": code += "  return UTILS.mat(dron);\n}"
            ...
            code += f"  return UTILS.mat(dron.subtract(CSG.cylinder({{start:{st}, end:{en}, radius:{R}, slices:128}})));\n}}"
        elif h == "stl_mount":
            w = p['stlm_w']; d = p['stlm_d']
            code += f"  var m1 = CSG.cube({{center:[{d/2},0,0], radius:[{w/2},15,3]}}).subtract(CSG.cylinder({{start:[{d/2},0,-5], end:[{d/2},0,5], radius:2.2, slices:32}}));\n"
            code += f"  var m2 = CSG.cube({{center:[{-d/2},0,0], radius:[{w/2},15,3]}}).subtract(CSG.cylinder({{start:[{-d/2},0,-5], end:[{-d/2},0,5], radius:2.2, slices:32}}));\n"
            code += f"  return UTILS.mat(dron.union(m1).union(m2));\n}}"
        elif h == "stl_ears":
            r = p['stle_r']; d = p['stle_d']
            code += f"  var c1=CSG.cylinder({{start:[{d/2},{d/2},0], end:[{d/2},{d/2},0.4], radius:{r}, slices:64}}); var c2=CSG.cylinder({{start:[{-d/2},{d/2},0], end:[{-d/2},{d/2},0.4], radius:{r}, slices:64}});\n"
            code += f"  var c3=CSG.cylinder({{start:[{d/2},{-d/2},0], end:[{d/2},{-d/2},0.4], radius:{r}, slices:64}}); var c4=CSG.cylinder({{start:[{-d/2},{-d/2},0], end:[{-d/2},{-d/2},0.4], radius:{r}, slices:64}});\n"
            ...
            code += f"  var out = CSG.cylinder({{start:[{px},{py},0], end:[{px},{py},10], radius:{r+t}, slices:128}});\n  var inn = CSG.cylinder({{start:[{px},{py},-1], end:[{px},{py},11], radius:{r}, slices:128}});\n"
            code += f"  return UTILS.mat(dron.union(out.subtract(inn)));\n}}"

    # -----------------------------------------
    # 3. GENERADORES PARAMÉTRICOS 3D (OVERDRIVE)
    # -----------------------------------------
    elif h == "laser":
        code += f"  var w = {p['las_x']}; var l = {p['las_y']}; var z_cut = {p['las_z']};\n"
        code += f"  var base_obj = CSG.cube({{center:[0,0,10], radius:[w/2, l/2, 10]}}).subtract(CSG.cylinder({{start:[0,0,-1], end:[0,0,21], radius:5, slices:64}}));\n"
        ...
        code += f"      var pieza = CSG.cylinder({{start:[px,py,0], end:[px,py,h], radius:5, slices:64}});\n"
        ...
        code += f"      var pieza = CSG.cylinder({{start:[px,py,0], end:[px,py,h], radius:r_pieza, slices:64}});\n"
        code += f"      if(!array_obj) array_obj = pieza; else array_obj = array_obj.union(pieza);\n  }}\n"
        code += f"  var base = CSG.cylinder({{start:[0,0,0], end:[0,0,h/2], radius:radio_corona + r_pieza + 2, slices:128}});\n"
        ...
        code += f"  var res = 60; var dz = h / res; var loft_obj = null; var hueco = null;\n"
        code += f"  for(var i=0; i<res; i++) {{\n      var z = i * dz; var t = i / res; var slice_res = 128;\n"
        ...
        code += f"          var p_ext = CSG.cylinder({{start:[x_curr, y_curr, z], end:[x_curr, y_curr, z+dz+0.1], radius:wall/2, slices:32}});\n"
        code += f"          var p_int = CSG.cylinder({{start:[x_int, y_int, z], end:[x_int, y_int, z+dz+0.1], radius:wall/4, slices:16}});\n"
        ...
        code += f"  var pipe = CSG.cylinder({{start:[0,0,0], end:[0,0,h], radius:r_out, slices:128}}).subtract(CSG.cylinder({{start:[0,0,-1], end:[0,0,h+1], radius:r_in, slices:128}}));\n"
        ...
        code += f"          var hole = CSG.sphere({{center:[cx, cy, z], radius:r_esfera, resolution:32}});\n"
        ...
        code += f"  var gear = CSG.cylinder({{start:[0,0,0], end:[0,0,h], radius:r_root, slices:256}});\n"
        ...
        code += f"      var t1 = CSG.cylinder({{start:[cx1,cy1,0], end:[cx1,cy1,h], radius:t_w*0.6, slices:32}});\n"
        code += f"      var t2 = CSG.cylinder({{start:[cx2,cy2,0], end:[cx2,cy2,h], radius:t_w*0.4, slices:32}});\n"
        code += f"      var t3 = CSG.cylinder({{start:[cx3,cy3,0], end:[cx3,cy3,h], radius:t_w*0.15, slices:32}});\n"
        code += f"      gear = gear.union(t1).union(t2).union(t3);\n  }}\n"
        code += f"  var hole = CSG.cylinder({{start:[0,0,-1], end:[0,0,h+1], radius: r_root * 0.3, slices:128}});\n"
        ...
        code += f"  var res = 30; var dz = h / res; var gear = null; var m = rb / (dientes/2);\n"
        code += f"  for(var z=0; z<res; z++) {{\n      var z_pos = z * dz; var r_curr = rb - (rb - rt)*(z/res); var r_root = Math.max(0.1, r_curr - m);\n"
        code += f"      var core = CSG.cylinder({{start:[0,0,z_pos], end:[0,0,z_pos+dz], radius:r_root, slices:128}});\n"
        ...
        code += f"          var t1 = CSG.cylinder({{start:[cx1,cy1,z_pos], end:[cx1,cy1,z_pos+dz], radius:t_w*0.6, slices:16}});\n"
        code += f"          var t2 = CSG.cylinder({{start:[cx2,cy2,z_pos], end:[cx2,cy2,z_pos+dz], radius:t_w*0.3, slices:16}});\n"
        code += f"          gear = gear.union(t1).union(t2);\n      }}\n  }}\n"
        code += f"  var hole = CSG.cylinder({{start:[0,0,-1], end:[0,0,h+1], radius: rt * 0.3, slices:64}});\n"
        ...
        code += f"  var pieza = CSG.cylinder({{start:[0,0,0], end:[0,0,h], radius:rint, slices:128}});\n"
        ...
        code += f"     var punta = CSG.cylinder({{start:[px, py, 0], end:[px, py, h], radius:r_punta, slices:64}});\n"
        ...
        code += f"  var res = 100; var dz = h / res; var solido = null; var hueco = null;\n"
        code += f"  for(var i=0; i<res; i++) {{\n      var z = i * dz; var f = Math.sin((z/h) * Math.PI); var rad = r1 + (r2 - r1)*(z/h) + (f * 15);\n"
        code += f"      var capa = CSG.cylinder({{start:[0,0,z], end:[0,0,z+dz], radius:rad, slices:128}});\n"
        code += f"      if(!solido) solido = capa; else solido = solido.union(capa);\n"
        code += f"      if (grosor > 0 && z > grosor) {{\n         var r_int = Math.max(0.1, rad - grosor);\n"
        code += f"         var capa_h = CSG.cylinder({{start:[0,0,z], end:[0,0,z+dz+0.1], radius:r_int, slices:128}});\n"
        ...
        rint = p['p_rint']; c = max(int(p['p_lados']), 128)
        ...
        if p['l_chaf'] > 0: code += f"  var fillet = CSG.cylinder({{start:[t, 0, t], end:[t, w, t], radius:chaf, slices:64}}); pieza = pieza.union(fillet);\n"
        if p['l_hueco'] > 0: code += f"  var h1 = CSG.cylinder({{start:[l*0.7, w/2, -1], end:[l*0.7, w/2, t+1], radius:r, slices:64}});\n  var h2 = CSG.cylinder({{start:[-1, w/2, l*0.7], end:[t+1, w/2, l*0.7], radius:r, slices:64}});\n  pieza = pieza.subtract(h1).subtract(h2);\n"
        code += f"  return UTILS.mat(pieza);\n}}"
    elif h == "engranaje":
        code += f"  var dientes = {int(p['e_dientes'])}; var r = {p['e_radio']}; var h = {p['e_grosor']};\n"
        code += f"  var d_x = r*0.15; var d_y = r*0.2;\n  var pieza = CSG.cylinder({{start:[0,0,0], end:[0,0,h], radius:r, slices:128}});\n"
        code += f"  for(var i=0; i<dientes; i++) {{\n    var a = (i * Math.PI * 2) / dientes;\n"
        code += f"    var diente = CSG.cube({{center:[Math.cos(a)*r, Math.sin(a)*r, h/2], radius:[d_x, d_y, h/2]}}); pieza = pieza.union(diente);\n  }}\n"
        if p['e_eje'] > 0: code += f"  var hueco = CSG.cylinder({{start:[0,0,-1], end:[0,0,h+1], radius:{p['e_eje']} + G_TOL, slices:64}}); pieza = pieza.subtract(hueco);\n"
        ...
        code += f"  for(var i=0; i<4; i++) {{\n    var cyl = CSG.cylinder({{start:[m[i][0]*dx, m[i][1]*dy, 0], end:[m[i][0]*dx, m[i][1]*dy, h-2], radius: 3.5, slices:32}});\n"
        code += f"    var hole = CSG.cylinder({{start:[m[i][0]*dx, m[i][1]*dy, 2], end:[m[i][0]*dx, m[i][1]*dy, h], radius: 1.5 + (G_TOL/2), slices:32}});\n"
        code += f"    pieza = pieza.union(cyl).subtract(hole);\n  }}\n  return UTILS.mat(pieza);\n}}"
    elif h == "vslot":
        code += f"  var l = {p['v_l']};\n  var pieza = CSG.cube({{center:[0,0,l/2], radius:[10,10,l/2]}});\n"
        code += f"  var ch = CSG.cylinder({{start:[0,0,-1], end:[0,0,l+1], radius:2.1 + (G_TOL/2), slices:64}}); pieza = pieza.subtract(ch);\n"
        ...
        code += f"  var fix = CSG.cylinder({{start:[0,0,0], end:[0,0,l/3], radius:d/2, slices:64}});\n"
        code += f"  var fix2 = CSG.cylinder({{start:[0,0,2*l/3], end:[0,0,l], radius:d/2, slices:64}});\n"
        code += f"  var move = CSG.cylinder({{start:[0,0,l/3+G_TOL], end:[0,0,2*l/3-G_TOL], radius:d/2, slices:64}});\n"
        code += f"  var pin = CSG.cylinder({{start:[0,0,l/3-d/4], end:[0,0,2*l/3+d/4], radius:(d/4)-G_TOL, slices:64}});\n"
        code += f"  var cut_pin = CSG.cylinder({{start:[0,0,l/3-d/2], end:[0,0,2*l/3+d/2], radius:d/4, slices:64}});\n"
        ...
        code += f"  var ext = CSG.cylinder({{start:[0,0,0], end:[0,0,ancho], radius:(diam/2)+grosor, slices:128}});\n"
        code += f"  var int_cyl = CSG.cylinder({{start:[0,0,-1], end:[0,0,ancho+1], radius:diam/2 + G_TOL, slices:128}});\n"
        ...
        code += f"  var m3 = CSG.cylinder({{start:[ distPestana, 10, ancho/2 ], end:[ distPestana, -10, ancho/2 ], radius:1.7 + (G_TOL/2), slices:32}});\n"
        code += f"  var m3_2 = CSG.cylinder({{start:[ -distPestana, 10, ancho/2 ], end:[ -distPestana, -10, ancho/2 ], radius:1.7 + (G_TOL/2), slices:32}});\n"
        ...
        if l_tornillo == 0: 
            code += f"  var m = {m}; var h = {h_cabeza};\n  var cuerpo = CSG.cylinder({{start:[0,0,0], end:[0,0,h], radius:{r_hex}, slices:6}});\n  var agujero = CSG.cylinder({{start:[0,0,-1], end:[0,0,h+1], radius:({r_eje} + G_TOL), slices:64}});\n  return UTILS.mat(cuerpo.subtract(agujero));\n}}"
        else: 
            code += f"  var m = {m}; var l_tornillo = {l_tornillo}; var h_cabeza = {h_cabeza}; var r_hex = {r_hex};\n  var cabeza = CSG.cylinder({{start:[0,0,0], end:[0,0,h_cabeza], radius:r_hex, slices:6}});\n  var eje = CSG.cylinder({{start:[0,0,h_cabeza - 0.1], end:[0,0,h_cabeza + l_tornillo], radius:({r_eje} - G_TOL) - (m*0.08), slices:64}});\n  var pieza = cabeza.union(eje); var paso = m * 0.15;\n  for(var z = h_cabeza + 1; z < h_cabeza + l_tornillo - 1; z += paso*1.5) {{\n      var anillo = CSG.cylinder({{start:[0,0,z], end:[0,0,z+paso], radius:({r_eje} - G_TOL), slices:32}});\n      pieza = pieza.union(anillo);\n  }}\n  return UTILS.mat(UTILS.rotZ(pieza, KINE_T));\n}}"
    elif h == "rodamiento":
        code += f"  var d_int = {p['rod_dint']}; var d_ext = {p['rod_dext']}; var h = {p['rod_h']};\n"
        code += f"  var pista_ext = CSG.cylinder({{start:[0,0,0], end:[0,0,h], radius:d_ext/2, slices:256}}).subtract( CSG.cylinder({{start:[0,0,-1], end:[0,0,h+1], radius:(d_ext/2)-2 + G_TOL, slices:256}}) );\n"
        code += f"  var pista_int_base = CSG.cylinder({{start:[0,0,0], end:[0,0,h], radius:(d_int/2)+2 - G_TOL, slices:256}}).subtract( CSG.cylinder({{start:[0,0,-1], end:[0,0,h+1], radius:d_int/2, slices:256}}) );\n"
        ...
        code += f"      var bola = CSG.sphere({{center:[bx, by, h/2], radius:(r_espacio*0.95) - (G_TOL/2), resolution:64}});\n"
        ...
        code += f"  var sol_base = CSG.cylinder({{start:[0,0,0], end:[0,0,h], radius:r_sol - 1, slices:128}});\n"
        code += f"  var dientes_sol = Math.floor(r_sol * 1.5);\n"
        code += f"  for(var i=0; i<dientes_sol; i++) {{\n      var a = (i * Math.PI * 2) / dientes_sol;\n"
        code += f"      var diente = CSG.cylinder({{start:[Math.cos(a)*r_sol, Math.sin(a)*r_sol, 0], end:[Math.cos(a)*r_sol, Math.sin(a)*r_sol, h], radius:1.2, slices:32}});\n"
        code += f"      sol_base = sol_base.union(diente);\n  }}\n"
        code += f"  var sol = UTILS.rotZ(sol_base.subtract(CSG.cylinder({{start:[0,0,-1], end:[0,0,h+1], radius:3, slices:64}})), T);\n"
        code += f"  var planetas = null; var dientes_planeta = Math.floor(r_planeta * 1.5);\n"
        code += f"  for(var p=0; p<3; p++) {{\n      var ap = (p * Math.PI * 2) / 3;\n"
        code += f"      var planeta = CSG.cylinder({{start:[0, 0, 0], end:[0, 0, h], radius:r_planeta - 1 - G_TOL, slices:128}});\n"
        ...
        code += f"          var diente_p = CSG.cylinder({{start:[px, py, 0], end:[px, py, h], radius:1.2 - (G_TOL/2), slices:32}});\n"
        code += f"          planeta = planeta.union(diente_p);\n      }}\n"
        code += f"      planeta = planeta.subtract(CSG.cylinder({{start:[0, 0, -1], end:[0, 0, h+1], radius:2, slices:32}}));\n"
        ...
        code += f"  var corona = CSG.cylinder({{start:[0,0,0], end:[0,0,h], radius:r_anillo + 5, slices:256}}).subtract(CSG.cylinder({{start:[0,0,-1], end:[0,0,h+1], radius:r_anillo + G_TOL, slices:256}}));\n"
        ...
        code += f"      var diente_c = CSG.cylinder({{start:[Math.cos(a)*(r_anillo + G_TOL), Math.sin(a)*(r_anillo + G_TOL), 0], end:[Math.cos(a)*(r_anillo + G_TOL), Math.sin(a)*(r_anillo + G_TOL), h], radius:1.2, slices:32}});\n"
        ...
        code += f"  var cuerpo = CSG.cylinder({{start:[0,0,1.5], end:[0,0,1.5+ancho], radius:r_ext, slices:128}});\n"
        code += f"  var matriz_dientes = null;\n"
        code += f"  for(var i=0; i<dientes; i++) {{\n      var a = (i * Math.PI * 2) / dientes;\n"
        code += f"      var d = CSG.cylinder({{start:[Math.cos(a)*r_ext, Math.sin(a)*r_ext, 1], end:[Math.cos(a)*r_ext, Math.sin(a)*r_ext, 2+ancho], radius:0.55, slices:16}});\n"
        ...
        code += f"  var base = CSG.cylinder({{start:[0,0,0], end:[0,0,1.5], radius:r_ext + 1, slices:128}});\n"
        code += f"  var tapa = CSG.cylinder({{start:[0,0,1.5+ancho], end:[0,0,3+ancho], radius:r_ext + 1, slices:128}});\n"
        code += f"  var polea = base.union(cuerpo).union(tapa);\n"
        code += f"  polea = polea.subtract(CSG.cylinder({{start:[0,0,-1], end:[0,0,5+ancho], radius:r_eje + (G_TOL/2), slices:64}}));\n  return UTILS.mat(UTILS.rotZ(polea, KINE_T));\n}}"
    elif h == "helice":
        code += f"  var rad = {p['hel_r']}; var n = {int(p['hel_n'])}; var pitch = {p['hel_p']};\n"
        code += f"  var hub = CSG.cylinder({{start:[0,0,0], end:[0,0,10], radius:8, slices:64}});\n"
        code += f"  var agujero = CSG.cylinder({{start:[0,0,-1], end:[0,0,11], radius:2.5 + G_TOL, slices:32}});\n"
        code += f"  var aspas = null;\n"
        code += f"  for(var i=0; i<n; i++) {{\n    var a = (i * Math.PI * 2) / n; var dx = Math.cos(a); var dy = Math.sin(a);\n"
        code += f"    var aspa = CSG.cylinder({{start:[6*dx, 6*dy, 5 - (pitch/10)], end:[rad*dx, rad*dy, 5 + (pitch/10)], radius: 3, slices: 16}});\n"
        ...
        code += f"  var bola = CSG.sphere({{center:[0,0,0], radius:r_bola, resolution:64}}); var eje_bola = CSG.cylinder({{start:[0,0,0], end:[0,0,-r_bola*2], radius:r_bola*0.6, slices:64}});\n"
        code += f"  var componente_bola = UTILS.rotY(UTILS.rotX(bola.union(eje_bola), KINE_T/4), KINE_T/4);\n"
        code += f"  var copa_ext = CSG.cylinder({{start:[0,0,-r_bola*0.2], end:[0,0,r_bola*1.5], radius:r_bola+4, slices:64}});\n"
        code += f"  var hueco_bola = CSG.sphere({{center:[0,0,0], radius:r_bola+G_TOL, resolution:64}}); var apertura = CSG.cylinder({{start:[0,0,r_bola*0.5], end:[0,0,r_bola*2], radius:r_bola*0.8, slices:64}});\n"
        ...
        code += f"      var post = CSG.cylinder({{start:[px,py,t], end:[px,py,t+h_post], radius:r_post, slices:32}});\n"
        code += f"      var hole = CSG.cylinder({{start:[px,py,t], end:[px,py,t+h_post+1], radius:r_hole + (G_TOL/2), slices:32}});\n"
        ...
        code += f"          var agujero = CSG.cylinder({{start:[vx,vy,-1], end:[vx,vy,t+1], radius:2, slices:16}});\n"
        ...
        code += f"  var resorte = null; var pasos = Math.floor(vueltas * 60); var paso_z = h / pasos; var a_step = (Math.PI * 2 * vueltas) / pasos;\n"
        ...
        code += f"      var seg = CSG.cylinder({{start:[x1,y1,z1], end:[x2,y2,z2], radius:r_hilo, slices:16}});\n"
        code += f"      var esp = CSG.sphere({{center:[x2,y2,z2], radius:r_hilo, resolution:16}});\n"
        ...
        code += f"  var r_core = r - (pitch * 0.4); var eje = CSG.cylinder({{start:[0,0,0], end:[0,0,len], radius:r_core, slices:64}});\n"
        code += f"  var thread = null; var steps = Math.floor((len / pitch) * 40); var z_step = len / steps; var a_step = (Math.PI * 2 * (len/pitch)) / steps; var w = pitch * 0.35;\n"
        code += f"  for(var i=0; i<steps; i++) {{\n      var a1 = i * a_step; var a2 = (i+1) * a_step; var z1 = i * z_step; var z2 = (i+1) * z_step;\n"
        code += f"      var seg = CSG.cylinder({{start:[Math.cos(a1)*r, Math.sin(a1)*r, z1], end:[Math.cos(a2)*r, Math.sin(a2)*r, z2], radius:w, slices:16}});\n"
        ...
        code += f"  var codo = null; var pasos = Math.max(16, Math.floor(angulo / 3));\n"
        ...
        code += f"      var ext = CSG.cylinder({{start:[x1,y1,0], end:[x2,y2,0], radius:r_tubo, slices:32}});\n"
        code += f"      var esf = CSG.sphere({{center:[x2,y2,0], radius:r_tubo, resolution:32}});\n"
        ...
        code += f"         var x1 = Math.cos(a1)*r_curva; var y1 = Math.sin(a1)*r_curva; var x2 = Math.cos(a2)*r_curva; var y2 = Math.sin(a2)*r_curva;\n"
        code += f"         var int_c = CSG.cylinder({{start:[x1,y1,0], end:[x2,y2,0], radius:r_tubo-grosor, slices:24}});\n"
        code += f"         var isf = CSG.sphere({{center:[x2,y2,0], radius:r_tubo-grosor, resolution:24}});\n"
        ...
        code += f"  var ala = null; var num_pasos = 80;\n"
        ...
        code += f"      var cyl = CSG.cylinder({{start:[x_real, 0, 0], end:[x_real, 0, envergadura], radius: yt_real, slices: 32}});\n"
        ...
        code += f"  var anillo = CSG.cylinder({{start:[0,0,t], end:[0,0,t+w], radius:(d/2)+t, slices:64}});\n"
        code += f"  var hueco = CSG.cylinder({{start:[0,0,t-1], end:[0,0,t+w+1], radius:(d/2), slices:64}});\n"
        ...
        code += f"  var pillar = CSG.cylinder({{start:[0,0,40], end:[0,0,150], radius: s/4, slices:64}});\n"
        code += f"  var top = CSG.cylinder({{start:[0,0,150], end:[0,0,160], radius: (s/4)+10, slices:64}});\n"
    ...
    # -----------------------------------------
        ...
        code += f"  var core = CSG.cylinder({{start: [0,0,0], end: [0,0,thick], radius: center_r, slices: 64}});\n"
        ...
        code += f"  var hole = CSG.cylinder({{start: [0,0,-1], end: [0,0,thick+1], radius: center_r * 0.6, slices: 64}});\n"
        ...
        code += f"      var pad = CSG.cylinder({{start: [px, py, 0], end: [px, py, thick], radius: m_pad_r, slices: 32}});\n"
        code += f"      var m_hole = CSG.cylinder({{start: [px, py, -1], end: [px, py, thick+1], radius: m_pad_r * 0.3, slices: 32}});\n"
        ...
        code += f"  var hub = CSG.cylinder({{start: [0,0,0], end: [0,0,h], radius: 6, slices: 32}}).subtract(CSG.cylinder({{start: [0,0,-1], end: [0,0,h+1], radius: 2.5, slices: 32}}));\n"
        code += f"  var prop = hub;\n"
    ...
    # -----------------------------------------
    # 5. DEMOSTRACIÓN DE MOTOR: PLANETARIOS Y ESPACIO (GPU OVERDRIVE)
    # -----------------------------------------
    elif h == "solar":
        code += f"  var s = {p.get('sol_s', 50)} / 50;\n"
        code += f"  var U_HD = 512; var HD = 256; var MD = 128; var SD = 64;\n"
        code += f"  var elementos = [];\n"
        code += f"  var sol = CSG.sphere({{center:[0,0,0], radius: 25*s, slices: U_HD}}).setColor([1.0, 0.95, 0.8]);\n"
        code += f"  elementos.push(sol);\n"
        code += f"  function crearOrbita(dist) {{\n"
        code += f"      return CSG.cylinder({{start: [0,0,-0.1], end: [0,0,0.1], radius: dist + 0.15, slices: HD}}).subtract(CSG.cylinder({{start: [0,0,-0.2], end: [0,0,0.2], radius: dist - 0.15, slices: HD}})).setColor([0.15, 0.15, 0.2]);\n"
        code += f"  }}\n"
        code += f"  function crearPlaneta(dist, radio, vel, color, numLunas) {{\n"
        code += f"      var angulo = (KINE_T * vel) * (Math.PI / 180);\n"
        code += f"      var px = Math.cos(angulo) * dist; var py = Math.sin(angulo) * dist;\n"
        code += f"      elementos.push(crearOrbita(dist));\n"
        code += f"      elementos.push(CSG.sphere({{center: [px, py, 0], radius: radio * s, slices: HD}}).setColor(color));\n"
        code += f"      if(numLunas > 0) {{ for(var i=1; i<=numLunas; i++) {{ var angLuna = (KINE_T * vel * (5 + i*3)) * (Math.PI / 180); var distLuna = (radio * s) + (i * 4) + 1; elementos.push(CSG.sphere({{center: [px + Math.cos(angLuna)*distLuna, py + Math.sin(angLuna)*distLuna, 0], radius: 0.7 * s, slices: MD}}).setColor([0.7, 0.7, 0.75])); }} }}\n"
        code += f"  }}\n"
        code += f"  crearPlaneta(45, 1.2, 4.1, [0.6, 0.5, 0.5], 0);\n"
        code += f"  crearPlaneta(65, 2.8, 1.6, [0.9, 0.7, 0.3], 0);\n"
        code += f"  crearPlaneta(95, 3.2, 1.0, [0.1, 0.6, 1.0], 1);\n"
        code += f"  crearPlaneta(130, 1.7, 0.5, [0.9, 0.3, 0.1], 2);\n"
        code += f"  for(var a=1; a<=1200; a++) {{\n"
        code += f"      var rand1 = (a * 13.54) % (Math.PI * 2); var rand2 = (a * 7.21) % 25; var rand3 = ((a * 3.14) % 8) - 4;\n"
        code += f"      var rDist = 155 + rand2; var ax = Math.cos(rand1) * rDist; var ay = Math.sin(rand1) * rDist;\n"
        code += f"      elementos.push(CSG.sphere({{center: [ax, ay, rand3], radius: (0.2 + (a%3)*0.1) * s, slices: SD}}).setColor([0.4, 0.4, 0.45]));\n"
        code += f"  }}\n"
        code += f"  crearPlaneta(220, 8.5, 0.2, [0.8, 0.6, 0.4], 8);\n"
        code += f"  var angSat = (KINE_T * 0.1) * (Math.PI / 180); var satX = Math.cos(angSat) * 310; var satY = Math.sin(angSat) * 310;\n"
        code += f"  elementos.push(CSG.sphere({{center: [satX, satY, 0], radius: 7.0 * s, slices: HD}}).setColor([0.9, 0.8, 0.5]));\n"
        code += f"  var tilt = 25 * (Math.PI / 180); var hExt = 0.1; var dzExt = hExt * Math.cos(tilt); var dyExt = hExt * Math.sin(tilt); var hInt = 0.3; var dzInt = hInt * Math.cos(tilt); var dyInt = hInt * Math.sin(tilt);\n"
        code += f"  var anilloA = CSG.cylinder({{start: [satX, satY + dyExt, -dzExt], end: [satX, satY - dyExt, dzExt], radius: 16 * s, slices: U_HD}}).subtract(CSG.cylinder({{start: [satX, satY + dyInt, -dzInt], end: [satX, satY - dyInt, dzInt], radius: 13.5 * s, slices: U_HD}})).setColor([0.85, 0.75, 0.65]);\n"
        code += f"  var anilloB = CSG.cylinder({{start: [satX, satY + dyExt, -dzExt], end: [satX, satY - dyExt, dzExt], radius: 13 * s, slices: U_HD}}).subtract(CSG.cylinder({{start: [satX, satY + dyInt, -dzInt], end: [satX, satY - dyInt, dzInt], radius: 9 * s, slices: U_HD}})).setColor([0.7, 0.6, 0.5]);\n"
        code += f"  elementos.push(anilloA); elementos.push(anilloB); elementos.push(crearOrbita(310));\n"
        code += f"  crearPlaneta(400, 4.0, 0.05, [0.4, 0.8, 0.9], 5);\n"
        code += f"  crearPlaneta(480, 3.8, 0.03, [0.1, 0.3, 0.8], 3);\n"
        code += f"  var out = []; for(var e=0; e<elementos.length; e++) out = out.concat(elementos[e].toPolygons());\n"
        code += f"  return UTILS.mat(CSG.fromPolygons(out));\n}}"

    elif h == "tierra":
        code += f"  var r_tierra = {p.get('ear_r', 40)}; var d_luna = {p.get('ear_d', 80)};\n"
        code += f"  var tierra = CSG.sphere({{center:[0,0,0], radius: r_tierra, resolution: 256}});\n"
        code += f"  var a = KINE_T % 360;\n"
        code += f"  var px = Math.cos(a * Math.PI/180) * d_luna; var py = Math.sin(a * Math.PI/180) * d_luna;\n"
        code += f"  var luna = CSG.sphere({{center:[px, py, 0], radius: r_tierra * 0.27, resolution: 128}});\n"
        ...
        code += f"  var sat_body = CSG.cylinder({{start:[sx, 0, sz - 2], end:[sx, 0, sz + 2], radius: 2, slices: 16}});\n"
        ...
        code += f"  var core = CSG.cylinder({{start:[0,0,-30], end:[0,0,30], radius: 10, slices: 128}});\n"
        code += f"  var ring = CSG.cylinder({{start:[0,0,-4], end:[0,0,4], radius: r, slices: 256}}).subtract(CSG.cylinder({{start:[0,0,-5], end:[0,0,5], radius: r - 6, slices: 256}}));\n"
        ...
        code += f"      var spoke = CSG.cylinder({{start:[0,0,0], end:[px*2, py*2, 0], radius: 3, slices: 16}});\n"
        code += f"      station = station.union(spoke);\n"
        code += f"      var mod_px = Math.cos(a) * r; var mod_py = Math.sin(a) * r;\n"
        code += f"      var modulo = CSG.sphere({{center:[mod_px, mod_py, 0], radius: 8, resolution: 64}});\n"
